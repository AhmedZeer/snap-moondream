...
def load_weights_from_safetensors(weights_file: str, model: nn.Module) -> None:
    ...
    with safetensors_open(weights_file) as get_tensor:
        ...
        # Layout 1 (current, 2025-06-21+): checkpoint keys match the model's
        # own state_dict names directly (e.g. "model.vision.blocks.0.attn.qkv").
        if (
            "model.vision.blocks.0.attn.proj.bias" in keys
            or "vision.blocks.0.attn.proj.bias" in keys
        ):
            tensors = {
                k.replace("model.", "", 1) if k.startswith("model.") else k: get_tensor(k)
                for k in keys
            }
            model.load_state_dict(tensors, strict=False)
            return
        # Layout 2 (legacy): original training checkpoint key names that need
        # remapping via _load_weights.
        name_map = {k.replace("._orig_mod", ""): k for k in keys}
